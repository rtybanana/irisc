...
Registers::Registers(int x, int y) : Fl_Group(x, y, 240, 540), registers {}, labels {}, label_borders {}, cpsr {}, flags {} {
  ...
  Fl::lock();
    ...
    for(auto const& [name, index] : syntax::regMap){
      ...
      Fl_Box* val_border = new Fl_Box(40, 10+(25*index), 190, 25);
    ...
    Fl_Box* flagbg = new Fl_Box(10, 420, 220, 25);
    flagbg->box(FL_FLAT_BOX);
    flagbg->color(vm::darker);
    
    Fl_Box* flagstitle = new Fl_Box(10, 420, 100, 25, "CPSR");
    flagstitle->box(FL_BORDER_FRAME);
    flagstitle->labelfont(FL_BOLD);
    flagstitle->labelcolor(FL_WHITE);
    flagstitle->labelsize(13);

    widgets::HoverBox* flagshover = new widgets::HoverBox(10, 420, 100, 25, "CPSR Flags", "Four bits in the Current Program Status Register which are used to decide whether an instruction should execute.");
    flagshover->callback(hover_cb, this);

    for (int i = 0; i < 4; i++) {
      FLAG f = static_cast<FLAG>(i);
      Fl_Box* flagname = new Fl_Box(110 + (i * 30), 420, 30, 25, flagShortName[f].c_str());
      ...
      Fl_Box* flag = new Fl_Box(110 + (i * 30), 420, 30, 25, "0");
      ...
      // explain on hover
      widgets::HoverBox* hover = new widgets::HoverBox(110 + (i * 30), 420, 30, 25, flagTitle[f], flagExplain[f]);
      hover->callback(hover_cb, this);
    }

    Fl_Box* explanation = new Fl_Box(10, 455, 220, 100);
    explanation->box(FL_BORDER_FRAME);
    _title = new Fl_Box(10, 455, 220, 20);
    _title->labelfont(FL_BOLD);
    _title->labelcolor(FL_WHITE);
    _title->labelsize(14);
    _title->align(FL_ALIGN_LEFT | FL_ALIGN_INSIDE);
    _details = new Fl_Box(10, 480, 220, 75);
